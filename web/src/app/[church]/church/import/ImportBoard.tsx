...
type Row = { name: string; suffix: string; phone: string; birthday: string; position: string };
  ...
  function parse() {
    const rows: Row[] = raw.split("\n").map((l) => l.trim()).filter(Boolean).map((line) => {
      ...
      return {
        name: m ? m[1] : cols[0] ?? "",
        suffix: m ? m[2] : "",
        phone: cols[1] ?? "", birthday: cols[2] ?? "", position: cols[3] ?? "",
      };
    }).filter((r) => r.name);
    setPreview(rows);
    setResult(null);
    ...
    <main className="max-w-2xl mx-auto p-4 flex flex-col gap-4">
      <div className="card p-5">
        ...
        <p className="text-sm text-[var(--text-soft)] mb-3">
          열 순서: <b>이름, 전화, 생년월일(YYYY-MM-DD), 직분</b> — 이름만 있어도 됩니다. 동명이인은 이름 끝에 A/B.
        </p>
        <textarea className="input !min-h-40 font-mono text-sm" value={raw}
                  placeholder={"김철수\t010-1234-5678\t1955-03-01\t집사\n이영희,010-2345-6789,,권사"}
          ...
          <div className="overflow-x-auto max-h-72 overflow-y-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-[var(--text-soft)]">
                ...
                <th className="font-bold">생일</th><th className="font-bold">직분</th></tr></thead>
              <tbody>
                {preview.map((r, i) => (
